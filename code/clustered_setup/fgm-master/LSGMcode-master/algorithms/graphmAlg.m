...
% get the thread id
id = getThreadID();
path_ = './algorithms/graphm-0.52/lsgm/';
...
dlmwrite([p 'temp\graph_A_' num2str(id) '.txt'], full(A(s+1:end,s+1:end)), '	');
dlmwrite([p 'temp\graph_B_' num2str(id) '.txt'], full(B(s+1:end,s+1:end)), '	');

% create config file
copyfile( [path_ 'config.txt'], [p 'temp\config' num2str(id) '.txt'] );
