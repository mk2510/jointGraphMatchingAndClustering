...
asgPmsdp2 = kpsdp_2_PMSDP_wrapper(asgT, K);
...
fprintf('\n KP-SVD-PMSDP solved obj: %.2f, optimal obj: %.2f\n'...
    , asgPmsdp2.solved_obj, asgPmsdp2.optimal_obj);
...
Ax = iniAx(1, rows, cols, [400 * rows, 900 * cols], 'hGap', .1, 'wGap', .1);
parCor = st('cor', 'ln', 'mkSiz', 7, 'cls', {'y', 'b', 'g'});
shAsgImg(Fs, gphs, asgPmsdp2, asgT, parCor, 'ax', Ax{1}, 'ord', 'n');
...
asgs = {asgT, asgGa, asgPm, asgSm, asgSmac, asgIpfpU, asgIpfpS, asgRrwm, asgFgmU, asgFgmD, asgPmsdp2};
rows = 2; cols = 6;
...
imshow(asgPmsdp2.X_pre, 'InitialMagnification',1000)
hold on; 
spy(asgT.X, 'go')

figure('Name','Spy post Projection','NumberTitle','off');
imshow(asgPmsdp2.X, 'InitialMagnification',1000)
hold on; 
spy(asgT.X, 'go')
