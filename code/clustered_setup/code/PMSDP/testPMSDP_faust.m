function testPMSDP_faust(mesh1num,mesh2num,saveDir)
%===============================================================
% module:
% ------
% testPMSDP_faust.m
...
% Description:
% -----------
% Example: Runs PM-SDP on the FAUST dataset
...
% data folder
addpath(genpath('~/Faust_preprocessed'))
% current folder
addpath(genpath(pwd))
...
% calculate errors
[errorNoSymmetry,errorWithSymmetry] = evaluateFaustAcceptSymmetry(mesh1num,mesh2num,1:5000,m1tom2idx);
...
mkdir(saveDir)
if doplot
    fname = sprintf('%s/FAUST_%d_to_%d.fig',saveDir,mesh1num, mesh2num);
    saveas(gcf,fname);
end
fname = sprintf('%s/FAUST_%d_to_%d.map',saveDir,mesh1num, mesh2num);
dlmwrite(fname, (m1tom2idx)');
fname = sprintf('%s/FAUST_%d_to_%d.mat',saveDir,mesh1num, mesh2num);
save(fname,'finalR','m1tom2idx','errorNoSymmetry','errorWithSymmetry','params');
end